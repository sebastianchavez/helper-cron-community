...
function getDb() {
    if (db)
        return db;
    const dbPath = path_1.default.join(electron_1.app.getPath('userData'), 'chat.db');
...
function initDb() {
    ...
    `);
        ...
        const tableInfo = db.prepare(`PRAGMA table_info(conversation)`).all();
    ...
    `);
        ...
        (0, user_profile_repository_1.initUserProfileTable)();
    }
    catch (error) {
